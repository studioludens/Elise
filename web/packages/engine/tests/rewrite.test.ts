...
import { MaxLengthExceededError, rewrite } from '../src/rewrite';
...
describe('rewrite', () => {
    ...
    test('Dragon curve at iteration 3', () => {
        ...
        expect(rewrite('FX', rules, 3)).toBe('FX+YF++-FX-YF++-FX+YF+--FX-YF+');
    });
    ...
    test('respects max length', () => {
        const rules = [{ axiom: 'F', rule: 'FF' }];
        expect(() => rewrite('F', rules, 20, { maxLength: 100 })).toThrowError(
