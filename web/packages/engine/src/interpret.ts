...
import { Turtle, type TurtleConfig } from "./turtle.js";

export type DrawEvent =
  | { kind: "line"; x0: number; y0: number; x1: number; y1: number; color: number; thickness: number }
...
export function interpret(system: string, options: InterpretOptions = {}): DrawEvent[] {
  ...
  while (i < system.length) {
    ...
    switch (ch) {
      ...
      case "+": t.rotateLeft(args.length === 1 ? args[0]! : angle); break;
      case "-": t.rotateRight(args.length === 1 ? args[0]! : angle); break;
      case "$": t.resetRotation(); break;
      case "!": t.lineLength *= args.length === 1 ? args[0]! : ratio; break;
      case "@": t.lineLength /= args.length === 1 ? args[0]! : ratio; break;
