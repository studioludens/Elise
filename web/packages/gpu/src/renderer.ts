 ...
 *   r.setCamera({ viewProj, viewport, scale });
...
/** Camera uniform: 4x4 view-proj (column-major) + viewport size + world scale. */
export type CameraUniform = {
    /** 16 floats (mat4x4). Column-major. */
    viewProj: Float32Array;
    /** [width, height] in pixels. */
    viewport: [number, number];
    /** World-units-per-CSS-pixel (i.e. view.scale). Used in the shader so that
     *  segment thickness is interpreted in CSS pixels, independent of zoom. */
    scale: number;
};
