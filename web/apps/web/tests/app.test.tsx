...
describe('App', () => {
    ...
    test('reset to preset restores axiom after edits', () => {
        ...
        expect((screen.getByLabelText(/^Axiom$/i) as HTMLInputElement).value).toBe(dragon.axiom);
    });
    ...
    test('changing angle slider updates the value field', () => {
        ...
        expect((screen.getByLabelText(/Angle value/i) as HTMLInputElement).value).toBe('45');
    });
    ...
    test('save .lsys then open .lsys round-trips axiom + rules', async () => {
        ...
        (file as unknown as { text: () => Promise<string> }).text = () => Promise.resolve(savedXml);
    ...
    test('restores state from localStorage on mount', () => {
        ...
        expect(selectEl.options[selectEl.selectedIndex]!.textContent).toMatch(/Saved \(custom\)/);
    });

    test('backend toggle swaps WebGPU for Canvas2D; WebGPU is the default and reports unsupported in jsdom', () => {
        render(<App />);
        expect(screen.getByTestId('lsystem-webgpu-canvas')).toBeInTheDocument();
        ...
        const canvas2d = screen.getByLabelText(/Canvas2D/i) as HTMLInputElement;
        fireEvent.click(canvas2d);
        expect(screen.queryByTestId('lsystem-webgpu-canvas')).not.toBeInTheDocument();
